...
int AddFriend(Person *p_friend, int count) {
	...
	if (count < MAX_COUNT) {
		...
		printf("\nt새로운 친구 정보를 입력하세요\n");
		printf("1. 이름: ");
		scanf_s("%s", p_friend-> sizeof(p_friend.name));

		printf("2. 나이: ");
		scanf_s("%hu",&p_friend->age); //age 배열의 count위치에 나이를 저장함.
...
int main(void) {
	...
	//무한 루프: 사용자가 3을 누르면 break문으로 종료시킴
	while (1) {
		//메뉴를 화면에 출력함
		printf("\t[ 메뉴 ]\t\n");
		printf("=======================\n");
		...
		case 1:
			...
			따라서 1을 반환했을때만 등록된 친구 수를 증가시킵니다.
			*/
